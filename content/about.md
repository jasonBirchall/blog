...
I built this site the way I wish more of the web worked, which mostly means leaving things out. There's no JavaScript, no cookies, no trackers, and no ads. The logs strip IP addresses and user agents before they're written, so your visit isn't recorded anywhere, including here. The feed at /feed.xml carries full posts, because a feed that makes you click through isn't a feed.

The stack is small on purpose: Django 5, SQLite, one Hetzner box, rootless Podman. Everything you're reading started as a Markdown file in git, and the site rebuilds itself by pulling signed commits, so the server holds no credentials worth stealing. Two hundred lines of hand-written CSS cover the lot.

It's all free software: source on Codeberg, code under MIT, prose under CC BY-SA 4.0. Take what's useful.
